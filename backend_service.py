...
SUMMARY_SOURCE = REPO_ROOT / "summary_net.md"
CHECKPOINT_SOURCE = REPO_ROOT / "models" / "best_model_net.pth"
...
if not DATASET_ROOT_ENV:
    raise RuntimeError("DATASET_ROOT must be set")
if not ADVERTISED_HOST:
    raise RuntimeError("ADVERTISED_HOST must be set")

DATASET_ROOT = Path(DATASET_ROOT_ENV).resolve()
